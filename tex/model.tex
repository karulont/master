...
between timesteps $t$ and $t+1$. First are the lift and drop variables for the
current timestep $t$.
\begin{align}
    &\A_{\mathrm{lift}} = \begin{cases}
        \{\lift_{u,w,t}\} &\colon w \in \Wl\\
        \emptyset &\colon w \notin \Wl
    \end{cases}
    &\A_{\mathrm{drop}} = \begin{cases}
        \{\drop_{u,w,t}\} &\colon w \in \Wd\\
        \emptyset &\colon w \notin \Wd
    \end{cases}
\end{align}
...
\begin{align}
    \B_{\mathrm{lift}} &= \begin{cases}
        \{\lift_{u,w_l,t-5} \mid w_l \in \Wl\} &\colon w = \stat{lft} \wedge
        t \in \T_{-5}\\
        \emptyset &\colon \text{otherwise}
    \end{cases}\\
    \B_{\mathrm{drop}} &= \begin{cases}
        \{\drop_{u,w_d,t-1} \mid w_d \in \Wd\} &\colon w = \stat{drp} \wedge
        t \in \T_{-1}\\
        \emptyset &\colon \text{otherwise}
    \end{cases}
\end{align}
Now there are the movement variables. Only continue and stop variables actually
correspond to node status changes.
\begin{align}
    \B_{\mathrm{move}} &= \begin{cases}
        \{\cont_{u,w_m,d+2,t} \mid w_m \in \Wm \wedge \getMC(w_m) = w\} &\colon (\edg(u,d),u) \in \E\\
        \emptyset &\colon \text{otherwise}
    \end{cases}\\
...
\begin{align}
    \begin{split}
        ...
        &C(1,w) \go_{v,w,d,t} + C_{2,w} \cont_{v,w,d,t} + C_{3,w} \stp_{v,w,d,t}\\
...
but we are more concerned about time. Therefore a combination of
both~(\ref{eq:energyobj}) and (\ref{eq:timeobj}) would be the best.
